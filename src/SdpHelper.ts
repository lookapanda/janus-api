...
export class SdpHelper {
     ...
     */
    public filterH264Profiles(
        sdp: string,
        allowedProfiles: Array<[]> | string | RegExp
    ) {
        ...
        for (const section of sections) {
            if (SDPUtils.getMid(section) === 'video') {
                ...
                rtpSections[0].map((l: any) => newSection.push(l));
                for (let j = 1; j < rtpSections.length; j++) {
                    ...
                    if (parsed && parsed.name === 'H264') {
                        const fmtp = rtpSection
                            .filter((l: any) => l.indexOf('a=fmtp:') === 0)
                            .shift();
                ...
                newSection.map((l: any) => ret.push(l));
            } else {
                SDPUtils.splitLines(section).map((l: any) => ret.push(l));
            }
        }
    ...
    public filterDirectCandidates(sdp: any, force: boolean = false) {
    ...
    public isDirectCandidate(candidateLine: any) {
