...
export class Janus<T extends WebSocketInterface> {
    ...
    public async addPlugin(plugin: JanusPlugin<T>) {
        ...
        const json: any = await this.transaction('attach', request, 'success');
        if (json.janus !== 'success') {
    ...
    public transaction(
        ...
        return new Promise(
            (resolve: PromiseResolve<any>, reject: PromiseReject) => {
                if (timeoutMs) {
                    setTimeout(() => {
                        reject(
                            new Error(
                ...
                const request = {
                    ...
                    ...{
                        janus: type,
                        session_id:
                            (payload && parseInt(payload.session_id, 10)) ||
                            this.sessionId,
    ...
    public send(type: string, payload: any) {
        return new Promise(
            (resolve: PromiseResolve<void>, reject: PromiseReject) => {
                ...
                this.ws.send(JSON.stringify(request), {}, (err: any) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve();
