...
class SdpHelper {
    ...
    filterH264Profiles(sdp, allowedProfiles) {
        const sections = sdp_1.default.splitSections(sdp);
        const ret = [];
        for (const section of sections) {
            if (sdp_1.default.getMid(section) === 'video') {
                const newSection = [];
                ...
                for (let j = 1; j < rtpSections.length; j++) {
                    ...
                    const parsed = sdp_1.default.parseRtpMap(rtpSection[0]);
                    if (parsed && parsed.name === 'H264') {
                        const fmtp = rtpSection
                        ...
                        if (Array.isArray(allowedProfiles)) {
                            for (const allowedProfile of allowedProfiles) {
                                isAllowed =
                                    isAllowed ||
                                        fmtp.indexOf('profile-level-id=' + allowedProfile) !== -1;
                            }
                        }
                        else if (typeof allowedProfiles === 'string') {
                            isAllowed =
                                fmtp.indexOf('profile-level-id=' + allowedProfiles) !== -1;
                        }
                ...
                sdp_1.default.splitLines(section).map((l) => ret.push(l));
            }
        }
        return ret.join('\r\n') + '\r\n';
    }
    filterDirectCandidates(sdp, force = false) {
        const lines = sdp_1.default.splitLines(sdp);
        const ret = [];
    ...
    isDirectCandidate(candidateLine) {
        const candidate = sdp_1.default.parseCandidate(candidateLine);
